...
from .newton_mr import newtonMR
...
def get_optim_and_schedulers(model, args):
    ...
    if args.gam_nonaccel:
        ...
        print("Using Newton\'s Minimum Residual Algorithm (newtonMR)")
        optimizer = newtonMR(params=model.parameters(), base_optimizer=base_optimizer, model=model,
                        adaptive=args.adaptive, args=args)
    else:
